## test the data and define conditions to start the app
           ...
           !isTRUE(test_data$whiteSpace)) {
            ...
            if(!is.null(input$IsSingleExperiment)){
                if(!isTRUE(input$IsSingleExperiment)) {
                    ...
                    wellsPerPlate <- feature_table2$data %>% group_by_(input$PlateDimension) %>% 
                        rename_(CSwellsB110 = input$WellDimension) %>% 
                        summarize(CSwellsB110_unqiue=n_distinct(CSwellsB110)) %>% 
                        select(CSwellsB110_unqiue) %>% unlist(use.names=F)
                    
                    if(length(unique(wellsPerPlate)) != 1)
                        test_data$wellForm = T
                    
                    if(sum(wellsPerPlate) != nrow(feature_table2$data))
